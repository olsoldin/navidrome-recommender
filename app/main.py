...
async def start_scan():
    ...
    asyncio.create_task(_run_scan(job_id, settings))
    return {"job_id": job_id}


async def _run_scan(job_id: str, settings) -> None:
    ...
    try:
        client = SubsonicClient(settings.navidrome_url, settings.navidrome_user, settings.navidrome_pass)
        result = await loop.run_in_executor(None, engine.build_recommendations, client, progress)
